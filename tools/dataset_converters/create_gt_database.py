...
class GTDatabaseCreater:
    ...
    def create(self):
        ...
        #     self.num_worker)
        loop_data = ((loop_dataset(i) for i in range(len(self.dataset))), len(self.dataset))
        multi_db_infos = []
        for data in track_iter_progress(loop_data):
            multi_db_infos.append(self.create_single(data))
