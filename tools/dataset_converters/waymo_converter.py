...
class Waymo2KITTI(object):
    ...
    def create_gt_bin(self):
        ...
        bin_save_path = os.path.dirname(self.load_dir)
        print(f'Now create gt.bin, will write to {bin_save_path}/gt.bin')
        objects = metrics_pb2.Objects()
        for file_idx in tqdm.tqdm(range(len(self))):
            pathname = self.tfrecord_pathnames[file_idx]
            dataset = tf.data.TFRecordDataset(pathname, compression_type='')
        ...
        f = open(f'{bin_save_path}/gt.bin', 'wb')
...
def create_ImageSets_img_ids(root_dir, splits):
    ...
    if len(splits) == 1 and splits[0] == 'validation':  # only process validation set
        idx_all = [[], []]
        path = join(root_dir, 'training', 'calib')
        if not exists(path):
            RawNames = []
        else:
        ...
        for i, split in enumerate(splits):
            split_dir = split if not split == 'validation' else 'training'
            path = join(root_dir, split_dir, 'calib')
            if not exists(path):
                RawNames = []
            else:
                RawNames = os.listdir(path)
